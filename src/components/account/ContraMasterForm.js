...
const ContraPaymentForm = ({ isEditMode = false, transactionId = null }) => {
  ...
  // Unified balance calculation function
  // const fetchAccountBalance = async (subheadId, isBankAccount = false) => {
  //   if (!subheadId) return 0;
  //   try {
  //     const response = await apiService.getdata(`generalledger/${formData.schoolUdise}`);
  //     const data = response.data || [];

  //     // Get opening balance
  //     const openingBalance = data.find(
  //       b => b.entryType === "Opening Balance" &&
  //         (b.custId && Number(b.custId.custId)) === Number(subheadId)
  //     )?.drAmt || 0;

  //     // Get all relevant transactions
  //     const transactions = data.filter(b =>
  //       (b.custId && Number(b.custId.custId)) === Number(subheadId) &&
  ...
  //       if (isBankAccount) {
  //         // For bank accounts
  //         if (t.entryType === "Bank Receipt" || t.entryType === "Cash Receipt" || t.entryType === "Contra Payment") {
  //           balance += t.drAmt || 0;
  //         } else if (t.entryType === "Bank Payment" || t.entryType === "Cash Payment" || t.entryType === "Contra Payment") {
  //           balance -= t.crAmt || 0;
  //         }
  //       } else {
  //         // For cash accounts
  //         if (t.entryType === "Cash Receipt" || t.entryType === "Bank Receipt" || t.entryType === "Contra Payment") {
  //           balance += t.drAmt || 0;
  //         } else if (t.entryType === "Cash Payment" || t.entryType === "Bank Payment" || t.entryType === "Contra Payment") {
  //           balance -= t.crAmt || 0;
  //         } 
  //       }
  //     });
  ...
  //     return balance;
  //   } catch (error) {
  //     console.error(`Error fetching balance for subhead ${subheadId}:`, error);
  //     return 0;
  //   }
  // };
  const fetchAccountBalance = async (subheadId, head, isBankAccount = false) => {

    const datas = await apiService.getdata('generalledger/')

    const opnNBalance = (datas.data || []).find(
      b => b.entryType === "Opening Balance" && (b.custId && Number(b.custId.custId)) === Number(subheadId)
    );
    // console.log(datas)
    // alert(opnNBalance.crAmt)
    const transBalance = (datas.data || []).filter(
      b => b.entryType === "Cash Receipt" || b.entryType === "Bank Reciept" || b.entryType === "Contra Payment" && (b.custId && Number(b.custId.custId)) === Number(subheadId)
    );
    console.log("transaction balance 2", transBalance)

    const transBalance2 = (datas.data || []).filter(
      b => b.entryType === "Cash Payment" || b.entryType === "Bank Payment" || b.entryType === "Contra Payment" && (b.custId && Number(b.custId.custId)) === Number(subheadId)
    )
    ...
    if (head === "Liabilities") {
      ...
      const transBalance = (datas.data || []).filter(
        b => b.entryType === "Contra Payment" && (b.custId && Number(b.custId.custId)) === Number(subheadId)
      );
      console.log("transaction balance 2", transBalance)
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      ...
      try {
        ...
        const cashAccount = allParties.find(p => p.custName.toLowerCase() === "cash in hand");
        if (cashAccount) {
  ...
  const handleAccountChange = async (e) => {
    ...
    const selectedAccount = contraAccounts.find(acc => acc.id === parseInt(value));

    if (name === 'payerAccountId') {
      setFormData(prev => ({ ...prev, payerAccountId: value, receiverAccountId: '' }));
      setReceiverBalance(0);

      if (selectedAccount) {
        const balance = await fetchAccountBalance(
          selectedAccount.subheadId, selectedAccount.headName,
  ...
  const validateForm = () => {
    if (!formData.payerAccountId) {
      setError('देनार खाते (From Account) आवश्यक आहे'); return false;
    }
    if (!formData.receiverAccountId) {
      setError('घेणार खाते (To Account) आवश्यक आहे'); return false;
    }
    if (formData.payerAccountId === formData.receiverAccountId) {
      setError('देनार आणि घेणार खाते वेगळे असणे आवश्यक आहे'); return false;
    }
    if (!formData.amount || parseFloat(formData.amount) <= 0) {
      setError('रक्कम शून्यापेक्षा जास्त असणे आवश्यक आहे'); return false;
    }
    if (!formData.narr) {
      setError('वर्णन/हेतू आवश्यक आहे'); return false;
    }

    // Check if both accounts are of same type (cash-to-cash not allowed)
    const payer = contraAccounts.find(acc => acc.id === parseInt(formData.payerAccountId));
    const receiver = contraAccounts.find(acc => acc.id === parseInt(formData.receiverAccountId));

    if (payer && receiver && payer.type === 'cash' && receiver.type === 'cash') {
      setError('रोख ते रोख व्यवहार करता येत नाही');
      return false;
    }

    if (payerBalance < parseFloat(formData.amount)) {
      Swal.fire('Warning', 'Transaction amount exceeds the available balance in the payer account.', 'warning');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const payerAccount = contraAccounts.find(acc => acc.id === parseInt(formData.payerAccountId));
      const receiverAccount = contraAccounts.find(acc => acc.id === parseInt(formData.receiverAccountId));
      ...
      Object.keys(formData).forEach(key => {
        if (![
          'img',
          'payerAccountId', 'receiverAccountId'].includes(key) && formData[key]) {
  ...
  const getReceiverAccounts = () => {
    ...
    return contraAccounts.filter(acc =>
    ...
    <div className="container-fluid py-3">
      <div className="row mb-3">
        <div className="col-12">
          <h3>{isEditMode ? 'परस्पर व्यवहार संपादित करा' : 'नवीन परस्पर व्यवहार (Contra Entry)'}</h3>
          <nav aria-label="breadcrumb">
            <ol className="breadcrumb">
              <li className="breadcrumb-item"><Link to="/account/transactions">व्यवहार</Link></li>
              <li className="breadcrumb-item active" aria-current="page">{isEditMode ? 'संपादित करा' : 'नवीन'} परस्पर व्यवहार</li>
            </ol>
          </nav>
        </div>
      </div>
      <div className="card">
        <div className="card-header"><h5 className="mb-0"><Repeat size={20} className="me-2" /> परस्पर व्यवहार तपशील प्रविष्ट करा</h5></div>
            ...
            <div className="row g-3 mb-3">
              <div className="col-md-4">
                ...
                <input type="date" id="entryDate" name="entryDate" className="form-control"
                  value={formData.entryDate} onChange={handleInputChange} required />
              </div>
            </div>
            ...
            <div className="mb-4 bg-light p-3 rounded border">
              <div className="row g-3">
                <div className="col-md-6">
                  <label htmlFor="payerAccountId" className="form-label fw-bold">From Account (देनार खाते) *</label>
                  <select id="payerAccountId" name="payerAccountId" className="form-select"
                    value={formData.payerAccountId} onChange={handleAccountChange} required>
                  ...
                  <input type="number" className="form-control" value={payerBalance} readOnly disabled />
                </div>

                <div className="col-md-6">
                  <label htmlFor="receiverAccountId" className="form-label fw-bold">To Account (घेणार खाते) *</label>
                  <select
                    id="receiverAccountId" name="receiverAccountId" className="form-select"
                    value={formData.receiverAccountId} onChange={handleAccountChange} required
                  ...
                  <label className="form-label mt-2">Current Balance</label>
                  <input type="number" className="form-control" value={receiverBalance} readOnly disabled />
                </div>
              </div>
            </div>

            <div className="mb-4 bg-light p-3 rounded border">
              <h5 className="border-bottom pb-2 mb-3 fw-bold"><BiIdCard className="me-2" /> देय रक्कम व इतर माहिती</h5>
              <div className="row g-3">
                <div className="col-md-4">
                ...
                <div className="col-md-8">
                  ...
                  <input type="text" id="narr" name="narr" className="form-control"
                    value={formData.narr} onChange={handleInputChange}
                    placeholder="उदा., बँकेतून रोख रक्कम काढली" required />
                </div>
                <div className="col-md-6">
                  <label htmlFor="img" className="form-label">डिपॉझिट स्लिप/पावती प्रत</label>
