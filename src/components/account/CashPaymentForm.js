...
const CashPaymentForm = ({ isEditMode = false, transactionId = null }) => {
  ...
  const head = async () => {
    ...
    const response = await apiService.getdata(
      `headmaster/getbyheadname/${headName}`
    );
  };
  ...
  const fetchCustomers = async () => {
    const response = await apiService.getdata(
      `customermaster/getcustomerbyheadname/Sundry%20Creditors/${schoolUdise}`
    );
    let selectedOpn = [];
    try {
    ...
    const filtered = (response.data || []).filter((party) =>
      selectedOpn.includes(party.subheadId.subheadId)
    );

    setParties(filtered);
    const response1 = await apiService.getdata(
      `customermaster/getcustomerbyheadname/Cash%20In%20Hand/${schoolUdise}`
    );

    const recordedMain = (response1.data || []).find(
      (c) =>
  ...
  useEffect(() => {
    ...
    const fetchInitialData = async () => {
      ...
      try {
        ...
        const nextVoucher = `CP-${new Date()
          .getFullYear()
          .toString()
          .slice(-2)}${(new Date().getMonth() + 1)
          .toString()
          .padStart(2, "0")}-00${Math.floor(Math.random() * 100) + 1}`;
        if (!isEditMode) {
  ...
  const handlePartyChange = (e) => {
    ...
    const init = async (selectedParty) => {
      ...
      const transBalance = (datas.data || []).filter(
        (b) =>
          ["Cash Payment", "Bank Payment", "Journal Payment"].includes(
            b.entryType
          ) &&
          b.subhead &&
          Number(b.subhead.subheadId) ===
            Number(selectedParty.subheadId.subheadId)
      );
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.amount || parseFloat(formData.amount) <= 0)
      errors.amount = "रक्कम 0 पेक्षा जास्त असावी.";
    if (!formData.narration.trim())
      errors.narration = "कृपया खर्चाचा तपशील लिहा.";
    ...
    <div className="container-fluid py-3">
      ...
      <nav aria-label="breadcrumb">
        <ol className="breadcrumb">
          <li className="breadcrumb-item">
            <Link to="/account/transactions/cash-payments">व्यवहार</Link>
          </li>
          <li className="breadcrumb-item active" aria-current="page">
            {isEditMode ? "संपादन" : "नवीन"} रोख पेमेंट
          </li>
        </ol>
      </nav>
      ...
      <div className="card">
        <div className="card-header">
          <h5 className="mb-0">रोख पेमेंट तपशील</h5>
        </div>
                ...
                <div className="col-md-4">
                  ...
                  <select
                    ...
                    className={`form-select ${
                      validationErrors.paidToId ? "is-invalid" : ""
                    }`}
              ...
              <div className="row g-3 mb-3">
                ...
                <div className="col-md-8">
                  ...
                  <input
                    ...
                    name="narration"
                    className={`form-control ${
                      validationErrors.narration ? "is-invalid" : ""
                    }`}
