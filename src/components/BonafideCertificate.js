...
function BonafideCertificate() {
    ...
    const { id: studentId } = useParams(); // Student's DB ID
    ...
    useEffect(() => {
        const fetchInitialReasons = async () => {
            ...
            try {
                ...
                setReasonOptions(["शैक्षणिक कामासाठी", "शासकीय योजनेसाठी", "शिष्यवृत्तीसाठी"]); // Fallback
            } finally {
                setIsLoading(false);
            }
        };
    ...
    useEffect(() => {
        const fetchStudentAndAcademicData = async () => {
            ...
            try {
                ...
                Swal.fire('Error', 'Failed to load student or academic details.', 'error');
                setIsDataLoaded(false);
            } finally {
                setIsLoading(false);
            }
    ...
    const handleGenerateAndPrint = async () => {
        ...
        if (!certificateReason || certificateReason.trim() === '') {
            Swal.fire('आवश्यक', 'कृपया प्रमाणपत्र कशासाठी पाहिजे ते नमूद करा.', 'warning');
            return;
        }
        ...
        try {
            ...
            if (isNewReason) {
                ...
                const bonafideResponse = await apiService.post("bonafide/", bonafidePayload);
            ...
            if (error.response) {
                if (error.response.status === 403) {
                    ...
                    errorMessage = `विनंतीमध्ये त्रुटी: ${error.response.data?.message || error.response.data || 'कृपया माहिती तपासा.'}`;
                } else {
    ...
    const CertificateTemplate = ({ isOfficeCopy = false }) => {
        if (!studentData || !academicData) return <div className="text-center p-5">प्रमाणपत्रासाठी माहिती लोड होत आहे...</div>;
            ...
            <div className="border border-dark p-2 h-100" style={{ position: 'relative' }}>
                ...
                <div className='pt-3'>
                    <p>
                        ...
                        माझ्या माहितीनुसार {studentData.gender === "पुरुष" ? ("त्याची") : ("तिची")} वर्तणूक चांगली आहे.
                        हे प्रमाणपत्र मागणीनुसार <span className='fw-bold'>{certificateReason || ' कारण नमूद नाही '}</span> कामी देण्यात येत आहे.
                    </p>
                </div>
    ...
    return (
        <div className='container mt-3 mb-3'>
            ...
            {isDataLoaded && studentData && academicData && certificateReason && certificateReason.trim() !== '' && (
                <>
            ...
            {isLoading && isDataLoaded && ( // Loader specifically for when "Generate and Print" is clicked
                <div className="text-center mt-3">
